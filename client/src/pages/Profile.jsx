...
import { updateUserStart, updateUserSuccess, updateUserFailure } from "../redux/userSlice/userSlice";
import app from "../firebase";
const Profile = () => {
  const [image, setImage] = useState(undefined);
  const [imagePercent, setImagePercnt] = useState(0);
  ...
  const handleImageUpload = async (image) => {
    ...
    uploadTask.on(
      ...
      (snapshot) => {
        ...
        setImagePercnt(Math.round(progress));
    ...
    <div className="p-3 max-w-lg mx-auto">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-5">
        ...
        <img
          ...
          className="h-24 w-24 self-center rounded-full cursor-pointer mt-2"></img>

        <p>
          {imageError ? (
            <span className="text-red-700">Error Uploading Image (file size must be less than 2 MB)</span>
          ) : (
            imagePercent > 0 && image < 100 ? (
